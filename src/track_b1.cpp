...
int main(int argc, char **argv){
    ros::init(argc, argv, "track_b1");
     ros::NodeHandle nh1("~");
 
    // ros::Publisher pose_pub = nh1.advertise<geometry_msgs::PointStamped>("/track_b1", 1000);
...
while (ros::ok()){
     ...
     if (inputLine.empty()) continue; // if there is an empty line then skip it
      ...
      geometry_msgs::PointStamped msg;
        ...
        ros::param::set("/track_b1_score", data.var2);
