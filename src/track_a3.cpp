...
int main(int argc, char **argv){
    ros::init(argc, argv, "track_a3");
     ros::NodeHandle nh1("~");
 
    ros::Publisher pose_pub = nh1.advertise<visualization_msgs::Marker>("/track_a3", 1000);
...
while (ros::ok()){
     ...
     if (inputLine.empty()) continue; // if there is an empty line then skip it
      ...
      ss >> Time>> Bus>> MessageID>> Message>> MessageLength;
        ...
        marker.scale.y = 0.2;
        marker.scale.z = 0.2;
                double track_b3_s=0.0;
        if (ros::param::get("/track_b3_score", track_b3_s))
        {
          if (track_b3_s > 50.0){
